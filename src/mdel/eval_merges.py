...
class ModelArguments:
    ...
    model_1_name_or_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
    ...
    model_2_name_or_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "The model checkpoint for weights initialization.Don't set if you want to train a model from scratch."
...
def main():
    ...
    elif model_args.model_1_name_or_path:
        config = AutoConfig.from_pretrained(model_args.model_1_name_or_path, **config_kwargs)
    else:
    ...
    if model_args.tokenizer_name:
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_args.model_1_name_or_path, **tokenizer_kwargs)
    else:
        raise ValueError(
    ...
    with torch.no_grad():
        for name, parameter in named_parameters_model.items():
            if name in named_parameters_model_2:
                # print(name)
                # print(model[name])
                parameter2 = named_parameters_model_2[name]
                parameter.data += parameter2.data
                parameter.data /= 2.0
    ...
    kwargs = {
        "finetuned_from": model_args.model_1_name_or_path,
