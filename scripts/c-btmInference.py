...
def findEnsembleWeights(embedder, query, clusterCenters, T):
  """
  ...
  """  
  ensembleWeights = []
  embedded_query = embedder(query)) # make sure embedder returns tensor
  pdist = torch.nn.PairwiseDistance(p=2)

  for cluster in clusterCenters:
    ensembleWeights.append(torch.exp(-1 * torch.pow(pdist(embedded_query, cluster),2) / T).mean())
...
def findModelProbabilities(embedder, query, clusterCenters, models, tokenizers, T, k):
  """
  ...
  modelProbs = 0
  ensembleWeights = findEnsembleWeights(embedder, query, clusterCenters, T)
...
def findNextToken(embedder, query, clusterCenters, models, tokenizers, T, k):
  """
  ...
  modelProbs = findModelProbabilities(embedder,query, clusterCenters, models, tokenizers, T, k)
  return tokenizers[0].decode(np.argmax(modelProbs)) # doesn't matter which tokenizer bc they're all the same



def generateSequence(embedder, query, end_token, clusterCenters, models, tokenizers, T, k, maxLength):
  """
  ...
  """
  currToken, currSequence = None, query
  while (len(currSequence) < maxLength) and (currToken != end_token):
    currToken = findNextToken(
        embedder, currSequence, clusterCenters, models, tokenizers, T, k)
